...
type IconIds = 'trophy' | 'database' | 'money';
...
export function CardDemo({ title, description, iconId }: { title: string; description: string; iconId: IconIds }) {
  return (
    <Card>
      <CardSkeletonContainer>
        <Skeleton iconId={iconId} />
      </CardSkeletonContainer>
...
const IconRenderer = ({ iconId, size = 24, color = 'currentColor' }: IconProps) => {
  const iconMap = {
    trophy: IconTrophy,
...
export const CardDescription = ({ children, className }: { children: React.ReactNode; className?: string }) => {
  return <p className={cn('text-md max-w-sm font-normal text-neutral-400', className)}>{children}</p>;
};
...
export const CardSkeletonContainer = ({
  ...
  return (
    <div
      className={cn(
        'z-40 h-[10rem] rounded-xl md:h-[15rem]',
        className,
        showGradient &&
