...
const Faq = ({ questionsData }: TFaqProps) => {
  ...
  return (
    <section className="flex max-w-7xl flex-col gap-8 p-4 md:gap-16">
      <h2 className="text-2xl font-bold text-slate-300 md:text-5xl">Frequently asked questions</h2>
      <ul className="flex w-full flex-col gap-2 border-b-2 border-b-neutral-200/80 dark:border-b-neutral-800/80">
        {questionsData.map((question, index) => {
          const isOpen = openIndex === index;

          return (
            <li key={index} className="flex flex-col border-t border-neutral-200 py-4 dark:border-neutral-800">
              <button
                onClick={() => toggleIndex(index)}
                className={`flex w-full cursor-pointer items-center justify-between text-left transition-colors ${
                  isOpen ? 'text-white dark:text-white' : 'text-neutral-600 dark:text-neutral-400'
                }`}
              >
                <h4 className="text-xl">{question.question}</h4>
                {isOpen ? <IconChevronUp className="text-white" /> : <IconChevronDown />}
              </button>
              ...
              <AnimatePresence initial={false}>
                {isOpen && (
                  ...
                  >
                    <p className="mt-2 text-base text-neutral-600 dark:text-neutral-400">{question.answear}</p>
                  </motion.div>
                )}
              </AnimatePresence>
            </li>
...
export const BUSINESS_QUESTIONS_DATA = [
  ...
  },
  {
    ...
    answear: 'Images, text, audio files, sensor data—any structured digital input that can be reviewed and verified.'
  },
  {
  ...
  {
    question: 'Is Quelldata compliant with data privacy and AI ethics standards?',
    answear:
...
export const USERS_QUESTIONS_DATA = [
  ...
  },
  {
    ...
    answear: 'No. You can participate anonymously. Your reputation is based purely on the quality of your work.'
  },
  {
  ...
  },
  {
    ...
    answear: 'Yes. Many tasks will be optimized for mobile devices for accessibility and convenience.'
  },
  {
