...
const schema = z.object({
  ...
  email: z.string().email('Invalid email address').min(1, 'Company name is required')
});
    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          >
            <div className="relative w-full max-w-md rounded-xl bg-[#0a0a0a] p-6 shadow-lg">
              {/* Header */}
              <div className="flex items-center justify-between">
                <h2 className="text-xl font-bold text-neutral-200">Join the Waitlist</h2>
                <X className="size-5 cursor-pointer" onClick={onClose} />
              </div>

              <p className="mt-2 text-sm text-neutral-300">
                Be the first to access Quelldata. Get early updates, priority access, and help shape the future of AI
                data collection.
              </p>
              ...
              <form className="mt-6 space-y-5" onSubmit={handleSubmit}>
                ...
                <button
                  type="submit"
                  className="w-full transform cursor-pointer rounded-lg bg-white px-6 py-2 font-medium text-black transition-all duration-300 hover:-translate-y-0.5 hover:bg-gray-200"
                >
