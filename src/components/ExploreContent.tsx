  ...
  <div className="flex flex-col gap-10">
    ...
    <div className="flex gap-6 overflow-x-auto">
      {imageSources.map((image, index) => (
        <Image key={index} className="h-100 w-auto object-contain" src={image.src} alt={image.alt} />
      ))}
    </div>
  </div>
);
    ...
    content: (
      <DataContent
        description="Monitor the status of your dataset in the app dashboard. Once enough
          ...
          ready for download."
        imageSources={[{ src: completedRequestsScreen, alt: 'compelted requests screen' }]}
      />
    )
  }
];
